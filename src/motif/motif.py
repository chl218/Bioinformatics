...
class MotifAlgorithm:
    ...
    def randomized_motif_search_iteration(self, n_iters: int, dna_list: List[str], k: int) -> List[str]:
    ...
    def gibbs_sampler(self, dna_list: List[str], k: int, n_samples: int) -> List[str]:
        best_motifs = []
        for dna in dna_list:
            i = random.randint(0, len(dna) - k)
            best_motifs.append(dna[i:i+k])
    ...
    def gibbs_sampler_iter(self, n_iters: int, dna_list: List[str], k: int, n: int) -> List[str]:
